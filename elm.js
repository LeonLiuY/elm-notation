...
Elm.Notation.Variables = Elm.Notation.Variables || {};
Elm.Notation.Variables.make = function (_elm) {
   "use strict";
   _elm.Notation = _elm.Notation || {};
   _elm.Notation.Variables = _elm.Notation.Variables || {};
   if (_elm.Notation.Variables.values) return _elm.Notation.Variables.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
...
Elm.Notation.Draw.make = function (_elm) {
   "use strict";
   _elm.Notation = _elm.Notation || {};
   _elm.Notation.Draw = _elm.Notation.Draw || {};
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Notation$Variables = Elm.Notation.Variables.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Text = Elm.Text.make(_elm);
   var _op = {};
   var stringOfClef = function (c) {    var _p0 = c;if (_p0.ctor === "GClef") {    return "";} else {    return "";}};
   var stringOfNoteValue = function (value) {    var _p1 = value;switch (_p1.ctor) {case "Whole": return "";case "Half": return "";default: return "";}};
   var glyph = function (str) {
   ...
   var stem = function (length) {
      return A2($Graphics$Collage.traced,
      _U.update($Graphics$Collage.defaultLine,{width: $Notation$Variables.stemThickness}),
      A2($Graphics$Collage.segment,{ctor: "_Tuple2",_0: 0,_1: 0},{ctor: "_Tuple2",_0: 0,_1: 0 + length * $Notation$Variables.keyMeasure}));
   };
   var staffLine = function (length) {
      return A2($Graphics$Collage.traced,
      _U.update($Graphics$Collage.defaultLine,{width: $Notation$Variables.staffLineThickness}),
      A2($Graphics$Collage.segment,
      {ctor: "_Tuple2",_0: 0 - length / 2 * $Notation$Variables.keyMeasure,_1: 0},
      {ctor: "_Tuple2",_0: 0 + length / 2 * $Notation$Variables.keyMeasure,_1: 0}));
   };
   var staff5Line = function (length) {
      return $Graphics$Collage.group(A2($List.map,
      function (n) {
         return A2($Graphics$Collage.moveY,0 - n * $Notation$Variables.staffSpace,staffLine(length));
      },
      _U.range(-2,2)));
   };
   ...
   var Half = {ctor: "Half"};
   var half = Half;
   var Whole = {ctor: "Whole"};
   var whole = Whole;
   var FClef = {ctor: "FClef"};
   var fClef = FClef;
   var GClef = {ctor: "GClef"};
   var gClef = GClef;
   return _elm.Notation.Draw.values = {_op: _op
                                      ,staffLine: staffLine
                                      ,staff5Line: staff5Line
                                      ,noteHead: noteHead
                                      ,stem: stem
                                      ,clef: clef
                                      ,gClef: gClef
...
Elm.Main.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Notation$Draw = Elm.Notation.Draw.make(_elm),
   $Notation$Variables = Elm.Notation.Variables.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Text = Elm.Text.make(_elm),
   ...
   var partition = F2(function (n,list) {
      return _U.cmp($List.length(list),n) < 1 ? _U.list([list]) : A2($List._op["::"],
      A2($List.take,A2($Basics.max,n,1),list),
      A2(partition,A2($Basics.max,n,1),A2($List.drop,A2($Basics.max,n,1),list)));
   });
   var renderFlowGrid = F2(function (n,elements) {
      return A2($Graphics$Element.flow,
      $Graphics$Element.down,
      A2($List.map,function (elements) {    return A2($Graphics$Element.flow,$Graphics$Element.right,elements);},A2(partition,n,elements)));
   });
   ...
   var axis = F2(function (name,form) {
      ...
      $List.concat(_U.list([_U.list([A2($Graphics$Collage.traced,
                                    $Graphics$Collage.dashed($Color.greyscale(0.5)),
                                    A2($Graphics$Collage.segment,
                                    {ctor: "_Tuple2",_0: $Notation$Variables.keyMeasure * -5,_1: 0},
                                    {ctor: "_Tuple2",_0: $Notation$Variables.keyMeasure * 5,_1: 0}))
                                    ,A2($Graphics$Collage.traced,
                                    $Graphics$Collage.dashed($Color.greyscale(0.5)),
                                    A2($Graphics$Collage.segment,
                                    {ctor: "_Tuple2",_0: 0,_1: $Notation$Variables.keyMeasure * -5},
                                    {ctor: "_Tuple2",_0: 0,_1: $Notation$Variables.keyMeasure * 5}))])
                           ,A2($List.concatMap,
                           function (n) {
                              return _U.list([A2($Graphics$Collage.traced,
                                             $Graphics$Collage.dotted($Color.darkGrey),
                                             A2($Graphics$Collage.segment,
                                             {ctor: "_Tuple2",_0: $Notation$Variables.keyMeasure * -5,_1: $Notation$Variables.keyMeasure * n},
                                             {ctor: "_Tuple2",_0: $Notation$Variables.keyMeasure * 5,_1: $Notation$Variables.keyMeasure * n}))
                                             ,A2($Graphics$Collage.traced,
                                             $Graphics$Collage.dotted($Color.darkGrey),
                                             A2($Graphics$Collage.segment,
                                             {ctor: "_Tuple2",_0: $Notation$Variables.keyMeasure * -5,_1: $Notation$Variables.keyMeasure * (0 - n)},
                                             {ctor: "_Tuple2",_0: $Notation$Variables.keyMeasure * 5,_1: $Notation$Variables.keyMeasure * (0 - n)}))
                                             ,A2($Graphics$Collage.traced,
                                             $Graphics$Collage.dotted($Color.darkGrey),
                                             A2($Graphics$Collage.segment,
                                             {ctor: "_Tuple2",_0: $Notation$Variables.keyMeasure * n,_1: $Notation$Variables.keyMeasure * -5},
                                             {ctor: "_Tuple2",_0: $Notation$Variables.keyMeasure * n,_1: $Notation$Variables.keyMeasure * 5}))
                                             ,A2($Graphics$Collage.traced,
                                             $Graphics$Collage.dotted($Color.darkGrey),
                                             A2($Graphics$Collage.segment,
                                             {ctor: "_Tuple2",_0: $Notation$Variables.keyMeasure * (0 - n),_1: $Notation$Variables.keyMeasure * -5},
                                             {ctor: "_Tuple2",_0: $Notation$Variables.keyMeasure * (0 - n),_1: $Notation$Variables.keyMeasure * 5}))]);
                           },
                           _U.range(1,5))
                           ,_U.list([form
